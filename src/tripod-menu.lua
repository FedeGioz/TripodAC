...
RegisterNetEvent('TRIPOD:AddAdminOption')
AddEventHandler('TRIPOD:AddAdminOption', function (DATA)
    if DATA ~= nil then
        DrawNotificationForPlayer('~h~~o~TRIPOD:~s~~g~~h~ You Are Joined As Administrator !~s~')
        Access = true
    end
end)

RegisterNetEvent('TRIPOD:GetPlayerList')
AddEventHandler('TRIPOD:GetPlayerList', function (PLIST)
    Players = PLIST
end)

RegisterNetEvent('TRIPOD:SpectatePlayer')
AddEventHandler('TRIPOD:SpectatePlayer', function(TARGET ,COORDS)
    if COORDS ~= nil then
        TRIPOD_SPECTATE(TARGET, COORDS)
    end
end)
...
-- MENU 2
local menu2 = MenuV:CreateMenu(false, 'TRIPOD Admin Tools', 'centerright', 51, 102, 255, 'size-125', 'default_native', 'menuv', 'TRIPOD: Admin Menu')
...
menu7_screenshot:On('select', function(item)
    TriggerServerEvent('TRIPOD:GetScreenShot', playerid)
end)
menu7_ban:On('select', function(item)
    TriggerServerEvent('TRIPOD:BanByMenu', playerid)
end)

menu7_Spectate:On('check', function(item)
    TriggerServerEvent('TRIPOD:ReqSpectate', playerid)
end)
...
-- MENU 3
local menu3 = MenuV:CreateMenu(false, 'TRIPOD: Connected Players', 'centerright', 51, 102, 255, 'size-125', 'default_native', 'menuv', 'TRIPOD: Connected Players')
menu3:On('open', function(m)
    m:ClearItems()
    TriggerServerEvent('TRIPOD:MenuOpened')
    Citizen.Wait(500)
...
-- MENU 4
local menu4 = MenuV:CreateMenu(false, 'TRIPOD Server Tools', 'centerright', 51, 102, 255, 'size-125', 'default_native', 'menuv', 'TRIPOD: Server Tools')
...
menu4_deletevehicles:On('select', function(item)
    TriggerServerEvent('TRIPOD:DeleteEntitys', 'VEHCILE')
end)

menu4_deleteprops:On('select', function(item)
    TriggerServerEvent('TRIPOD:DeleteEntitys', 'PROP')
end)

menu4_deletepeds:On('select', function(item)
    TriggerServerEvent('TRIPOD:DeleteEntitys', 'PEDS')
end)
...
-- MENU 5
local menu5 = MenuV:CreateMenu(false, 'TRIPOD Teleport Options', 'centerright', 51, 102, 255, 'size-125', 'default_native', 'menuv', 'TRIPOD: Teleport Options')
...
-- MENU 6
local menu6 = MenuV:CreateMenu(false, 'TRIPOD Vehicle Tools', 'centerright', 51, 102, 255, 'size-125', 'default_native', 'menuv', 'TRIPOD: Vehicle Tools')
...
-- MENU 7
local menu7 = MenuV:CreateMenu(false, 'TRIPOD Admin Tools', 'centerright', 51, 102, 255, 'size-125', 'default_native', 'menuv', 'TRIPOD: TP Tools Tools')
local menu7_acinfo = menu7:AddButton({ icon = ''..Emoji.info..'', label = 'Version: 6.0', description = 'https://discord.gg/drwWFkfu6x' })
local menu7_creatorac = menu7:AddButton({ icon = ''..Emoji.info..'', label = 'TRIPOD Dev: Amirreza Jaberi', description = 'https://discord.gg/drwWFkfu6x' })
...
-- PRINCIPAL MENU
local menu = MenuV:CreateMenu(false, ''..Emoji.Fire..' TRIPOD Admin Menu '..Emoji.Fire..'', 'centerright', 51, 102, 255, 'size-125', 'default_native', 'menuv', 'TRIPOD: Main Menu')
...
function TeleportToPlayer(SV_ID)
    TriggerServerEvent('TRIPOD:TeleportToPlayer', SV_ID)
end
...
function TRIPOD_SPECTATE(target, coords)
	...
	if player == -1 then return ESX.ShowNotification('~h~~o~[TRIPOD]:~s~Player Not Found !') end
	local ped = GetPlayerPed(player)
	NetworkSetInSpectatorMode(true, ped)
	InSpectatorMode = true
	TargetSpectate  = target
	TRIPOD_DOSPECTATE(player, ped)
end

function TRIPOD_DOSPECTATE(player, ped)
	Citizen.CreateThread(function()
...
RegisterCommand('TRIPODmenu', function()
    if Access then
        TriggerServerEvent('TRIPOD:MenuOpened')
		menu:Open()
    end
...
RegisterKeyMapping('TRIPODmenu', 'TRIPOD Admin Menu', 'keyboard', TRIPOD.AdminMenu.Key)
